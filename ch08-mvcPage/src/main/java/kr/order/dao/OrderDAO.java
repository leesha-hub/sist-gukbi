...
public class OrderDAO {
	...
	public void insertOrder(OrderVO order, 
		...
		try {
			...
			for(int i=0;i<orderDetailList.size();i++) {
				...
				//계속 추하하면 outOfMemory 발생, 1000개 단위로 executeBatch()
				if(i%1000==0) {
					pstmt3.executeBatch();
				}
			}//end of for
	...
	//개별 상품 목록
	public List<OrderDetailVO> getListOrderDetail(int order_num)
	                                        throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public void updateOrder(OrderVO order)throws Exception{
		...
		try {
			...
			/*
			 * 배송상태를 변경할 때 배송대기일 때만 주문정보를 변경할 수 있도록
			 * 처리. 배송준비중,배송중,배송완료로 변경한 후 다시 배송대기로 변경
			 * 하면 수정 폼에 주문 정보가 없어서 오류가 나는 현상이 발생하기 
			 * 때문에 DB에 저장된 내용을 확인해서 배송대기 중일 때만 주문정보 변경이
			 * 가능하도록 처리
			 */
