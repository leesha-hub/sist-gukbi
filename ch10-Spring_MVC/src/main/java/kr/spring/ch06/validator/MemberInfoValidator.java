...
public class MemberInfoValidator implements Validator{
	...
	public void validate(Object target, Errors errors) {
		...
		if(memberInfo.getId() == null || 
				        memberInfo.getId().isEmpty()) {
			                 //필드   에러코드   
			errors.rejectValue("id", "required");
		}
		if(memberInfo.getName() == null || 
				       memberInfo.getName().isEmpty()) {
			errors.rejectValue("name", "required");
		}
		if(memberInfo.getZipcode() == null ||
				      memberInfo.getZipcode().isEmpty()) {
			errors.rejectValue("zipcode", "required");
		}
		if(memberInfo.getAddress1() == null ||
				      memberInfo.getAddress1().isEmpty()) {
			errors.rejectValue("address1", "required");
		}
		if(memberInfo.getAddress2() == null ||
				     memberInfo.getAddress2().isEmpty()) {
